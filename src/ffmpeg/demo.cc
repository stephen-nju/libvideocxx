...
int main(int argc, const char* argv[]) {
    ...
    spdlog::info("opening the input file =={}== and loading format (container) header", argv[1]);
    ...
    spdlog::info("format %s, duration %lld us, bit_rate %lld",
    ...
    for (int i = 0; i < pFormatContext->nb_streams; i++) {
        ...
        spdlog::info("AVStream->start_time %" PRId64, pFormatContext->streams[i]->start_time);
        spdlog::info("AVStream->duration %" PRId64, pFormatContext->streams[i]->duration);
    ...
    while (response >= 0) {
        ...
        if (response >= 0) {
            ...
            if (pFrame->format != AV_PIX_FMT_YUV420P) {
                spdlog::info(
                    "Warning: the generated file may not be a grayscale image, but could e.g. "
